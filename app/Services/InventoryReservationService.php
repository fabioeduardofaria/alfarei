    ...
    public function reserveFor(ProductionOrder $production): void
    {
        ...
        foreach ($production->order->items as $item) {
            ...
            foreach ($item->product->materials as $material) {
                ...
                if ($available < $quantity) throw ValidationException::withMessages(['stock' => "Estoque insuficiente para {$locked->name}. Disponível: {$available} {$locked->unit}; necessário: {$quantity} {$locked->unit}."]);
