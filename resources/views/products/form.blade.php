...
@section('content')
    ...
    <form class="panel form-card product-form" method="POST" enctype="multipart/form-data" action="{{ $product->exists ? route('produtos.update', $product) : route('produtos.store') }}">
        ...
        <section class="form-section">
            ...
            <div class="form-grid">
                ...
                <label>Tipo<select name="type" id="productType"><option value="product" @selected(old('type', $product->type) === 'product')>Produto físico</option><option value="service" @selected(old('type', $product->type) === 'service')>Serviço</option><option value="virtual" @selected(old('type', $product->type) === 'virtual')>Produto virtual · arquivo digital</option></select></label>
        ...
        <section class="form-section digital-product-fields" id="digitalProductFields" @if(old('type', $product->type) !== 'virtual') hidden @endif>
            <div class="section-intro"><div><span>↓</span><h2>Arquivo digital</h2></div><p>DXF, SVG ou ZIP. O original fica privado e só é liberado ao comprador após o pagamento integral.</p></div>
            <div class="form-grid">
                <label>Versão do arquivo<input name="digital_version" value="{{ old('digital_version', $product->digital_version) }}" maxlength="40" placeholder="Ex.: 1.0">@error('digital_version')<small class="field-error">{{ $message }}</small>@enderror</label>
                <label>Arquivo para entrega<input type="file" name="digital_file" accept=".dxf,.svg,.zip"><small>Até 50 MB. Trocar o arquivo não remove versões já compradas.</small>@error('digital_file')<small class="field-error">{{ $message }}</small>@enderror</label>
                @if($product->digital_file_path)
                    <p class="span-2 digital-file-current">Arquivo atual: <b>{{ $product->digital_file_name }}</b> · {{ number_format(($product->digital_file_size ?? 0) / 1024 / 1024, 2, ',', '.') }} MB · <a href="{{ route('produtos.digital.download', $product) }}">Baixar para conferir</a></p>
                @endif
                <label class="span-2">Termos de uso da licença<textarea name="digital_license_terms" rows="4" maxlength="3000" placeholder="Descreva o uso permitido, a fabricação de peças físicas e as restrições de redistribuição do arquivo.">{{ old('digital_license_terms', $product->digital_license_terms) }}</textarea>@error('digital_license_terms')<small class="field-error">{{ $message }}</small>@enderror</label>
            </div>
            <p class="form-tip">Para publicar na loja, envie o arquivo e preencha a versão e os termos de uso. Produtos virtuais não exigem ficha técnica de produção nem frete.</p>
        </section>

        <section class="form-section">
            <div class="section-intro"><div><span>2</span><h2>Preço e custo</h2></div><p id="priceSectionDescription">O custo será recalculado pela ficha técnica quando materiais e operações forem cadastrados.</p></div>
            ...
            <p class="form-tip" id="technicalSheetHint">Depois de salvar, abra a <b>Ficha técnica</b> para compor o custo com materiais, tempo e máquinas.</p>
            <p class="form-tip" id="digitalCostHint" hidden>Para arquivos digitais, informe o preço de venda e o custo estimado de criação, revisão e plataforma por venda. Não há ficha técnica nem custo de produção física.</p>
        ...
        <section class="form-section">
            ...
            <div class="product-switches">
                <label class="option-card" id="madeToOrderOption"><input type="hidden" name="made_to_order" value="0"><input type="checkbox" name="made_to_order" value="1" @checked(old('made_to_order', $product->exists ? $product->made_to_order : true))><span><b>Produzido sob encomenda</b><small>Cria produção quando o pedido for liberado.</small></span></label>
                ...
                <label class="option-card" id="personalizationOption"><input type="hidden" name="allow_personalization" value="0"><input type="checkbox" name="allow_personalization" value="1" @checked(old('allow_personalization', $product->allow_personalization))><span><b>Aceita personalização</b><small>Exibe um campo para o cliente descrever o pedido.</small></span></label>
    ...
    @endif
    <script src="{{ asset('js/product-digital.js') }}?v={{ filemtime(public_path('js/product-digital.js')) }}" defer></script>
@endsection
