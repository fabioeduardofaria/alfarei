...
<form class="panel display-admin-form text-config-form" method="POST" action="{{ route('texts.config.update') }}">@csrf @method('PUT')
    ...
    <div class="display-fields display-fields-three">
        ...
        <label>Base de apoio por unidade (R$)<x-field-tip text="Custo completo para produzir uma base física que mantém o nome em pé, incluindo material, corte e montagem. Só é cobrado quando a opção Com base é escolhida. Se não houver custo adicional, informe zero."/><input type="number" name="base_cost_per_unit" value="{{ old('base_cost_per_unit', $configurator->base_cost_per_unit) }}" min="0" step="0.01" required></label>
    ...
    @else
        <form method="POST" action="{{ route('texts.config.simulate') }}" data-admin-simulator="text" data-result="textSimulationResult" data-error="textSimulationError">@csrf
            <div class="display-fields display-fields-three">
                ...
                <fieldset class="text-base-options text-sim-base"><legend>Como será usado?</legend><label><input type="radio" name="with_base" value="0" @checked((string) old('with_base', '0') === '0')><span><b>Sem base</b><small>Para colar no painel.</small></span></label><label><input type="radio" name="with_base" value="1" @checked((string) old('with_base', '0') === '1')><span><b>Com base de apoio</b><small>Para ficar em pé na mesa.</small></span></label></fieldset>
        ...
        <div class="text-simulation-result" id="textSimulationResult" aria-live="polite" @unless($simulation) hidden @endunless>
            <div class="text-simulation-heading"><div><small>SIMULAÇÃO · CLIENTE FINAL</small><h3 data-sim="title">@if($simulation){{ $simulation['text'] }} · {{ $simulation['material_name'] }} {{ $simulation['thickness_mm'] }} mm @endif</h3><p data-sim="details">@if($simulation){{ $simulation['finish_label'] }} · {{ $simulation['base_label'] }} · {{ number_format($simulation['width_cm'], 1, ',', '.') }} × {{ number_format($simulation['height_cm'], 1, ',', '.') }} cm @if($simulation['width_estimated']) · largura estimada @endif · {{ $simulation['quantity'] }} unidade(s)@endif</p></div><strong data-sim="total">@if($simulation)R$ {{ number_format($simulation['total'], 2, ',', '.') }}@endif</strong></div>
            ...
            <div class="text-simulation-breakdown simulation-cost-details" id="textCostDetails" hidden>
                @foreach(['materialCost' => 'Material', 'laserCost' => 'Máquina laser', 'finishCost' => 'Acabamento', 'base' => 'Base de apoio', 'packaging' => 'Embalagem', 'setup' => 'Preparação da arte'] as $key => $label)
