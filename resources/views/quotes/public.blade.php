...
<main class="page">
    ...
    <section class="card details"><div><h2>Condições</h2><p><strong>Entrada:</strong> {{ number_format($quote->deposit_percent, 0, ',', '.') }}%<br><strong>Prazo de produção:</strong> {{ $quote->production_lead_days ? $quote->production_lead_days.' dias úteis' : 'A combinar' }}</p><p>{!! nl2br(e($quote->payment_terms ?: 'Condições de pagamento a combinar com a equipe comercial.')) !!}</p></div><div><h2>Observações</h2><p>{!! nl2br(e($quote->notes ?: 'Proposta preparada especialmente para você.')) !!}</p><a class="button download" href="{{ route('proposta.pdf', $quote->approval_token) }}">Baixar PDF profissional</a></div></section>
